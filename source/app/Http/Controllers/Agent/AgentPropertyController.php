...
class AgentPropertyController extends Controller
{
    ...
    public function AgentAddProperty()
    {
        try {
            ...
            $latestPackCredit = '';
            $latestPackCredit = PackagePlan::where('user_id', $id)->latest()->pluck('package_credits')->first();

            // Corrected condition: check if $currentCredit is 1 or equal to $totalCredit
            if ( $latestPackCredit == 0 || $currentCredit == $latestPackCredit) {
            ...
            // Log the exception or handle it in a way that suits your application
            $notification = [
                'message' => 'An error occurred while AgentAddProperty. ' . $e->getMessage(),
                'alert-type' => 'error',
            ];
    ...
    public function AgentStoreProperty(Request $request)
    {
        ...
        try {
            ...
            $image = $manager->read($image)->resize(370, 250);
            // Save the manipulated image
            $image->toJpeg()->save('upload/property/thambnail/' . $name_gen);
    ...
    public function AgentEditProperty($encryptedId)
    {
        try {
            $id = decrypt($encryptedId);
            // Fetch facilities and property details
            ...
            // Fetch states, property types, and amenities for dropdowns
            ...
            return view('agent.property.edit_property', compact('country','states','cities','localAreas','property', 'propertytype', 'amenities', 'property_ami', 'multiImage', 'facilities', 'pstate'));
        } catch (\Exception $e) {
            // Log the exception or handle it in a way that suits your application
            $notification = [
                'message' => 'An error occurred while fetching AgentEditProperty. ' . $e->getMessage(),
                'alert-type' => 'error',
            ];
    ...
    public function AgentUpdateProperty(Request $request)
    {
            ...
            $validator = Validator::make($request->all(), [
                ...
                'lowest_price.regex' => 'Invalid lowest price format.',
    ...
    public function AgentPropertyMessage()
    {
        ...
        try{
            ...
            $id = Auth::user()->id;
            $usermsg = PropertyMessage::where('agent_id', $id)->orderBy('id','DESC')->get();
    ...
    public function AgentMessageDetails($encryptedId)
    {
        try{
            ...
            $updateMsg = $msgdetails->update(['agent_status' => 'read']);
    ...
    public function AgentMessageDetele($encryptedId)
    {
    ...
    public function AgentDeleteProperty($encryptedId)
    {
        ...
        try {
            ...
            // Delete the thumbnail image associated with the property
            unlink($property->property_thambnail);

            // Delete the property record
            Property::findOrFail($id)->delete();

            // Fetch and delete multi-images associated with the property
            ...
            // Fetch and delete facilities associated with the property
    ...
    public function AgentDetailsProperty($encryptedId)
    {
        try {
            $id = decrypt($encryptedId);
            // Fetch facilities and property details
            ...
            // Fetch property types and amenities for dropdowns
            $propertytype = PropertyType::latest()->get();
            $amenities = Amenities::latest()->get();
            ...
            // Log the exception or handle it in a way that suits your application
            $notification = [
                'message' => 'An error occurred while fetching AgentDetailsProperty. ' . $e->getMessage(),
                'alert-type' => 'error',
            ];
    ...
    public function BuyPackage()
    {
        try {
            ...
            $currentCredit = PackagePlan::where('user_id',$uid)->latest()->pluck('package_credits')->first();
            $userCredit= User::where('id',$uid)->pluck('credit')->first();

            $CurrentPlan = '';
            if ($userCredit != $currentCredit ) {

                $CurrentPlan = PackagePlan::where('user_id', $uid)->latest()->pluck('package_name')->first();
            }

            $PackBasic = package_plan_setting::where('package_name','Basic')->first();
            $PackBusiness = package_plan_setting::where('package_name','Business')->first();
            $PackProfessional = package_plan_setting::where('package_name','Professional')->first();
            return view('agent.package.buy_package',compact('currency_symbol','CurrentPlan','PackBasic','PackBusiness','PackProfessional'));

        } catch (\Exception $e) {
            // Log the exception or handle it in a way that suits your application
            $notification = [
                'message' => 'An error occurred while fetching BuyPackage. ' . $e->getMessage(),
                'alert-type' => 'error',
            ];
    ...
    public function BuyBasicPlan()
    {
        ...
        try {
            ...
            $PackSetting = package_plan_setting::where('package_name','Basic')->first();
            return view('agent.package.plan', compact('currency_symbol','data','sitesetting','invoice','PackSetting'));

        } catch (\Exception $e) {
            // Log the exception or handle it in a way that suits your application
            $notification = [
                'message' => 'An error occurred while fetching BuyBasicPlan. ' . $e->getMessage(),
                'alert-type' => 'error',
            ];
    ...
    public function BuyBusinessPlan()
    {
        ...
        try {
            ...
            $sitesetting = SiteSetting::find(1);
            $invoice = mt_rand(10000000, 99999999);
            $PackSetting = package_plan_setting::where('package_name','Business')->first();
            return view('agent.package.plan', compact('currency_symbol','data','invoice','sitesetting','PackSetting'));
    ...
    public function BuyProfessionalPlan()
    {
        try {
            ...
            $sitesetting = SiteSetting::find(1);
            $invoice = mt_rand(10000000, 99999999);
            $PackSetting = package_plan_setting::where('package_name','Professional')->first();
            return view('agent.package.plan', compact('currency_symbol','data','invoice','sitesetting','PackSetting'));
    ...
    public function StorePlan(Request $request)
    {
        try {
            ...
            $request->validate([
                ...
                'package_credits' => 'required|integer',
            ...
            $notification = array(
                'message' => 'You have purchase ' .$request->package_name. ' package successfully',
                'alert-type' => 'success'
            );
            ...
            // Log the exception or handle it in a way that suits your application
            $notification = [
                'message' => 'An error occurred while Plan purchase. ' . $e->getMessage(),
                'alert-type' => 'error',
            ];
    ...
    public function PackageHistory()
    {
        ...
        try{
            ...
            return view('agent.package.package_history', compact('packagehistory','currency_symbol'));

        } catch (\Exception $e) {
            // Log the exception or handle it in a way that suits your application
            $notification = [
                'message' => 'An error occurred while fetching PackageHistory. ' . $e->getMessage(),
                'alert-type' => 'error',
            ];
        ...
        try{
            ...
            if (!$packagehistory) {
             ...
             if (!$data) {
                ...
                $pdf = Pdf::loadView('agent.package.package_history_invoice', compact('currency_symbol','packagehistory','sitesetting','data'))->setPaper('a4')->setOption([
                    'tempDir' => public_path(),
                    'chroot' => public_path(),
                ]);
            ...
            // Log the exception or handle it in a way that suits your application
            $notification = [
                'message' => 'An error occurred while fetching Package invoice. ' . $e->getMessage(),
                'alert-type' => 'error',
            ];
    ...
    public function AgentScheduleRequest()
    {
        try{
            ...
            $id = Auth::user()->id;
            $usermsg = Schedule::where('agent_id', $id)->orderBy('id', 'desc')->get();
            ...
            $notification = [
                'message' => 'An error occurred while fetching AgentScheduleRequest. ' . $e->getMessage(),
                'alert-type' => 'error',
            ];
    ...
    public function AgentDetailsSchedule($encryptedId)
    {
        try{
            ...
            $notification = [
                'message' => 'An error occurred while fetching AgentDetailsSchedule. ' . $e->getMessage(),
                'alert-type' => 'error',
            ];
    ...
    public function AgentDeteleSchedule($encryptedId)
    {
