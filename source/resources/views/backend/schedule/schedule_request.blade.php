                        ...
                        <table id="dataTableExample" class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if(Auth::user()->can('schedule.details'))
                                        @php $encryptedId = encrypt($item->id  ?? ''); @endphp
                                        <a href="{{ route('admin.details.schedule',$encryptedId)}}"
                                            class="btn btn-inverse-info" title="Details"> <i data-feather="eye"></i>
                                        </a>
                                        @endif
                                        @if(Auth::user()->can('schedule.detele'))
                                        @php $encryptedId = encrypt($item->id  ?? ''); @endphp
                                        <a href="{{ route('admin.delete.schedule',$encryptedId) }}"
