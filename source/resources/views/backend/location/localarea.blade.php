...
@section('title','All Local Area')
@section('admin')
<div class="page-content">
    <nav class="page-breadcrumb">
        <ol class="breadcrumb">
            @if(Auth::user()->can('add.local_area'))
                        ...
                        <table id="dataTableExample" class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $item->name ?? ''}}</td>
                                    <td>
                                        @if(Auth::user()->can('edit.local_area'))
                                        <button type="button" class="btn btn-inverse-warning" data-bs-toggle="modal"
                                         ...
                                         @endif
                                        @if(Auth::user()->can('delete.local_area'))
                                        <a href="{{ route('delete.local.area',$item->id) }}"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel"> Add Local Area</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="btn-close"></button>
            </div>
            <div class="modal-body">
                <form method="POST" action="{{ route('store.local.area') }}" class="forms-sample" id="myForm" >
                    @csrf
...
<div class="modal fade" id="locedit" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" >
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="POST" action="{{ route('update.local.area') }}" class="forms-sample" id="myFormEdit">
                    @csrf
        ...
        $('#states').change(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    // Add a default option
                    $('#cities').append('<option value="" disabled selected>Select city</option>');
...
// Function to handle location editing
    function locEdit(id) {
        // Get city data
        $.getJSON('/local/area/' + id, function (data) {
            // Set values in form fields
            $('#loc_id').val(data.id);
            $('#name').val(data.name);

            // Pass the selected country ID to populateStateSelect
            populateCitySelect(data.city_id);
            //console.log('Console ids ' + data.city_id);
    ...
    // Function to populate the city select field
    function populateCitySelect(selectedCityId) {
        $.getJSON('/getCityShow', function (cities) {
            // Get the city select element
            var citySelect = $('#city_id');

            // Fetch the associated state_id for the selected city
            var selectedCity = cities.find(function (cityItem) {
                return cityItem.id == selectedCityId;
            });

            if (selectedCity) {
                var selectedStateId = selectedCity.state_id;

                // Call the function to populate the state select field
                populateStateSelect(selectedStateId);
            }

            // Filter cities based on the selected state_id
            var filteredCities = cities.filter(function (cityItem) {
                return cityItem.state_id == selectedStateId;
            });

            // Populate the city select field with filtered cities
            citySelect.html(filteredCities.map(function (cityItem) {
                // Create an option element for each city
                return $('<option>', {
                    value: cityItem.id,
                    text: cityItem.name,
                    selected: selectedCityId == cityItem.id,
                });
            }));
        }).fail(function (error) {
            console.error("Error fetching cities: " + error);
